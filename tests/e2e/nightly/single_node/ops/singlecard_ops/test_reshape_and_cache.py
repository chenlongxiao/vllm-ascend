...
def test_myops(num_tokens, num_head, block_size, num_blocks, count):
    ...
    warm_up = 0
    for _ in range(warm_up):
        group_len, group_key_idx, group_key_cache_idx = torch.ops._C_ascend.cache_by_group_pre(
    ...
    N = 101
    for zt_i in range(N):
        group_len, group_key_idx, group_key_cache_idx = torch.ops._C_ascend.reshape_and_cache_by_group(
            slot_mapping_npu, slot_mapping_list, block_size
        )
        torch.ops._C_ascend.store_kv_block(
            key_npu, key_cache_npu, group_len, group_key_idx, group_key_cache_idx, block_size
